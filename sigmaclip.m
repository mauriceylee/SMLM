function [thresholdtobeused,logicalforlowerthanthreshold] = sigmaclip(givendistribution,num_sigma,num_iterations)
%% Introduction
...
logicalforlowerthanthreshold = true(size(givendistribution));
% 1 is lower than threshold, 0 is higher than threshold
% threshold can only decrease with each iteration
for ii = 1:num_iterations
    median_distribution = median(givendistribution(logicalforlowerthanthreshold));
    sigma_distribution = std(givendistribution(logicalforlowerthanthreshold));
    
    % calculate the appropriate threshold for this iteration
    thresholdtobeused = median_distribution+num_sigma*sigma_distribution;
    % change the logical for higher values to zero
    logicalforlowerthanthreshold(givendistribution>thresholdtobeused) = 0;
...
% % % hold on
% % % plot([thresholdtobeused thresholdtobeused],[0 max(h38)],'r')
% % % hold off
