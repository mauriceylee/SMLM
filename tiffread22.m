...
%~~~~~~~~~~~~~~~~~~~~~~~~~~
%version 2.5, Kuan Song, Ph.D, June 10 2010, University of Maryland
%corrected the following bugs:
%1. Now truly reads tiff images with more than 3 layers. Previously only
%reads first three layers
%2. lastImageIndex was not correct, now updated
%3. function read_plane() needs to accomodate TIF.SampleFormat as an array

% Maurice Lee changed file name to tiffread22 and omitted some parts of
% this code to make it slightly faster
...
while (ifd_pos ~= 0)
    ...
    if (nargin<=1);  img_first = 1; img_last = TIF.SamplesPerPixel; end
    ...
    if ( img_skip + 1 >= img_first )
        ...
        if ( TIF.SamplesPerPixel == 1 )
            ...
            for ii = img_first:img_last
                IMG.data(:,:,ii)=read_plane(0, IMG.width, IMG.height, ii);
            end
        end
...
switch( uint8(TIF.SampleFormat(1)))
    case 1
